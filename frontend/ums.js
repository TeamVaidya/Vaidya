...
function fetchUsers(page = currentPage, size = pageSize) {
    fetch(`http://localhost:8081/users/pagination?page=${page}&size=${size}`)
...
function deleteUsers(userIds) {
    ...
    if (confirm('Are you sure you want to delete the selected users?')) {
        // Prepare delete promises for all user IDs
...
document.getElementById('filter-button').addEventListener('click', function() {
    ...
    if (!document.getElementById(filterRowId)) {
        ...
        filterButton.textContent = 'Apply'; // Change button text to 'Apply'
        ...
        filterButton.addEventListener('click', function applyFilter() {
            ...
            //filterButton.textContent = 'Apply'; // Change button text back to 'Filter'
            //filterButton.removeEventListener('click', applyFilter); // Remove the 'Apply' listener
        });
    }
});
function applyFilters() {
    // Collect filter values from the input fields
    ...
    // Check if any filters were applied; if not, alert the user
...
document.getElementById("remove-filters-button").addEventListener("click", function() {
    ...
    window.location.href = "ums.html";  // Redirect to ums.html
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    userTableBody.addEventListener('change', (e) => {
        ...
        if (selectedUsers.length > 0) {
            ...
            // If only one user is selected, show both Edit and Delete buttons
            if (selectedUsers.length === 1) {
                editButton.style.display = 'inline-block';
                deleteButton.style.display = 'inline-block';
            } 
            // If more than one user is selected, show only Delete button
    ...
    editButton.addEventListener('click', () => {
        // Find the selected user checkbox
